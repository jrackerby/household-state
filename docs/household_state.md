...
| `sensor.household_state_directive` | DIRECTIVE axis, plus the resolved banner cell (#30) | `reason`, `driver`, `suppressed` (always present, never omitted empty), `source_count`; the cell: `cell`, `gate`, `imperative`, `action`, `tone`, `stage_word`, `stage_on`, `stage_tone`, `quiet`, `evacuate`, `status`, `hazard_driver`, `hazard_source`, `hazard_name`, `hazard_window`, `masked`, `masked_by` — always present, resolved in `banner.py` after the fall dwell (see `DESIGN_CONTRACT.md`, "The rendering matrix") |
